...
* Add back definition of _`RECURSE-QUERY`_ that was inadvertantly dropped in R3.
...
> ::: add
>
> [4]{.pnum} For a type `Tag`, subexpression `sndr`, and pack `env`, let `CS` be
...
>   is ill-formed.
>
> [3]{.pnum} For a type `Tag`, subexpression `sndr`, and pack `env`, let `CS` be
> `completion_signatures_of_t<decay_t<decltype((sndr))>, decltype((env))...>`. If both
...
> types removed.
> 
> [7]{.pnum} For a type `Tag`, subexpression `sndr`, and pack `env`, ```@_`COMPL-DOMAIN`_@(Tag, sndr, env)```
...
>   constexpr decltype(auto) @_`call-with-default`_@(Fn&& fn, Default&& value, Args&&... args) noexcept(@_`see below`_@);
> ```
>
> [52]{.pnum} Let `e` be the expression
...
> [53]{.pnum} _Returns_: `e`.
>
> [54]{.pnum} _Remarks_: The expression in the `noexcept` clause is `noexcept(e)`.
>
> [55]{.pnum}
> ```cpp
> template<class Tag>
...
>
> [4]{.pnum} Given a type `Env`, if `completion_signatures_of_t<Sndr>` and
...
>
> [1]{.pnum} `schedule_from` offers scheduler authors a way to customize how to transition off of
...
>
> [2]{.pnum} The name `schedule_from` denotes a customization point object. For some subexpression
> `sndr`, if `decltype(sndr)` does not satisfy `sender`, `schedule_from(sndr)` is
> ill-formed.
>
> [3]{.pnum} Otherwise, the expression `schedule_from(sndr)` is expression-equivalent to
> ```@_`make-sender`_@(schedule_from, {}, sndr)```.
>
> :::
...
> :::
>
> [8]{.pnum} [Otherwise, ]{.add} [T]{.rm}[t]{.add}he expression
...
>
> [8]{.pnum} ```@_`impls-for`_@<@_`decayed-typeof`_@<@_`let-cpo`_@>>::@_`get-state`_@``` is
