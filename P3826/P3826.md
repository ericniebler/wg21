...
The section [](#removing-algorithm-customization) describes the effects and possible
remedies of the removal option.
...
uncomfortably close to the release of C++26. One mitigating factor is that it is unlikely
...
- ```@_`COMPL-DOMAIN`_@(T, S, E)``` is the type of `get_completion_domain<T>(get_env(S),
  E)` if that expression is well-formed, or `indeterminate_domain<>()` otherwise, and
...
#. Simplify the `on` algorithm, which no longer needs to use `write_env` to make child
   operations aware of the current scheduler and domain.
...
> ::: add
>
> 4. For a type `Tag`, subexpression `sndr`, and pack `env`, let `CS` be
...
> :::
>
> 5. Let _`completion-fn`_ be a completion function ([exec.async.ops]); let
...
>
>    - [2.1]{.pnum} ```@_`TRY-QUERY`_@(o, get_completion_domain<@_`completion-tag`_@>, env...)```
>
>    - [2.2]{.pnum} ```@_`TRY-QUERY`_@(get_completion_scheduler<@_`completion-tag`_@>(o, env...), get_completion_domain<set_value_t>, env...)```
>
>    <!--
>    - [2.3]{.pnum} `default_domain()` if `sizeof...(env) != 0` is `true`.
>    -->
>
>    If none of the above expressions is well-formed, the expression
>    ```get_completion_domain<@_`completion-tag`_@>(o, env...)```{.cpp} is ill-formed.
>
...
>    well-formed, then the expression
>    ```get_completion_scheduler<@_`completion-tag`_@>(get_env(schedule(sch)), env...)```{.cpp}
...
[Before [exec.snd.expos]{.sref} paragraph 6, add two new paragraph as follows:]{.ednote}
...
>    types removed.
> 
> 7. For type `Tag`, subexpression, `sndr` and pack `env`, ```@_`COMPL-DOMAIN`_@(Tag, sndr, env)```
...
[Replace [exec.snd.expos]{.sref} paragraph 6 about _`SCHED-ATTRS`_ and _`SCHED-ENV`_ with
the following paragraphs:]{.ednote}

> ::: add
>
> 8. For unqualified type `Tag` and subexpressions `sch`, `sndr`, and pack `as`,
>    ```@_`SCHED-ATTRS`_@(sch, sndr)``` is an expression `o` whose type satisfies
>    _`queryable`_ such that `o.query(Tag(), as...)` is expression-equivalent to:
>
>    - [8.1]{.pnum} `Tag()(sch, as...)` if `Tag` is a specialization of
>      `get_completion_scheduler_t` or `get_completion_domain_t` and either of the
>      following is `true`:
>
>      - [8.1.1]{.pnum} The template parameter of `Tag` is `set_value_t`, or
>
>      - [8.1.2]{.pnum}
>        ```@_`never-completes-with`_@<Sndr, @_`completion-tag`_@, decltype(as)...>```
>        is `true`, where _`completion-tag`_ is the template parameter of `Tag`,
>        and `Sndr` is `decay_t<decltype((sndr))>`.
>
>    - [8.2]{.pnum} `D()` if `Tag` is ```get_completion_domain_t<@_`completion-tag`_@>```,
>      where `D` is the type of the expression
>
>      > ```cpp
>      > @_`COMMON-DOMAIN`_@(@_`COMPL-DOMAIN`_@(@_`completion-tag`_@, sndr, as...),
>      >               @_`COMPL-DOMAIN`_@(@_`completion-tag`_@, schedule(sch), @_`FWD-ENV`_@(as)...))
>      > ```
>
>    - [8.3]{.pnum} Otherwise, ```@_`FWD-ENV`_@(get_env(sndr)).query(Tag(), as...)``` if
>      either of the following is `true`:
>
>      - [8.3.1]{.pnum} `Tag` is not a specialization of `get_completion_scheduler_t` or
>        `get_completion_domain_t`, or
>
>      - [8.3.2]{.pnum}
>        ```@_`never-completes-with`_@<schedule_result_t<decltype((sch))>, @_`completion-tag`_@, decltype(as)...>```
>        is `true`, where _`completion-tag`_ is the template parameter of `Tag`.
>
>    - [8.4]{.pnum} Otherwise, the expression `o.query(Tag(), as...)` is ill-formed.
>
> 9. For a scheduler `sch`, ```@_`SCHED-ENV`_@(sch)``` is an expression `o` whose type
>    satisfies _`queryable`_ such that
>
>    - [9.1]{.pnum} `o.query(get_scheduler)` is a prvalue with the same type and value as
>      `sch`,
>
>    - [9.2]{.pnum} `o.query(get_domain)` is expression-equivalent to
...
>    ```@[`static_cast<Sndr>(`]{.add}@std​::​forward<Sndr>(sndr)@[`)`]{.add}@```.
>    [See [https://cplusplus.github.io/LWG/issue4368](https://cplusplus.github.io/LWG/issue4368)]{.ednote}
>
> 3. _Returns_: `e`.
>
...
>    >       @@[```[](const auto& data) noexcept -> @_`inline-attrs`_@<@_`decayed-typeof`_@<@_`set-cpo`_@>> {```]{.add}@@
>    >         @@[```return {};```]{.add}@@
>    >       @@[```};```]{.add}@@
>    >
...
>    >     @@[```static constexpr auto @_`get-attrs`_@ =```]{.add}@@
>    >       @@[```[](const auto& data) noexcept -> @_`inline-attrs`_@<set_value_t> {```]{.add}@@
>    >         @@[```return {};```]{.add}@@
>    >       @@[```};```]{.add}@@
>    >
>    >     static constexpr auto @_`start`_@ =
...
>      expression-equivalent to ```get_completion_scheduler<@_`completion-tag`_@>(get_env(child), env2...)```.
>
>    - [5.2]{.pnum} ```attrs.query(get_completion_domain<@_`completion-tag`_@>, env...)``` is
>      expression-equivalent to ```get_completion_domain<@_`completion-tag`_@>(get_env(child), env2...)```.
>
>    - [5.3]{.pnum} For a subexpression `q` and pack `args`, ```attrs.query(q, args...)```
>      is expression-equivalent to ```@_`FWD-ENV`_@(get_env(child)).query(q, args...)```
>      where the type of `q` is not a specialization of either
...
>    >   @@[```return @_`SCHED-ATTRS`_@(data, child);```]{.add}@@
>    > }
>    > ```
>
...
>    > if constexpr (scheduler<decltype(data)>) {
>    >   auto orig_sch =
>    >     @_`call-with-default`_@(get_scheduler, @_`not-a-scheduler`_@(), env);
>    >
>    >   return continues_on(
>    >     starts_on(std::forward_like<OutSndr>(data), std::forward_like<OutSndr>(child)),
>    >     std::move(orig_sch));
>    > } else {
>    >   auto& [sch, closure] = data;
>    >   auto orig_sch = @_`call-with-default`_@(
>    >     get_completion_scheduler<set_value_t>, @_`not-a-scheduler`_@(), get_env(child), env);
>    >
>    >   return continues_on(
...
>    > ```
>
>    Given a pack of subexpressions `env`, let `Env` be the pack `decltype(env)`, let
>    `Data` be the decayed type of `data`, let `sndr` be a `const` lvalue reference to the
>    _`basic-sender`_ object that owns the objects to which `data` and `child` refer, and
>    let `new_attrs` be a _`queryable`_ object equal to
>    `get_env(on.transform_sender(set_value, sndr_copy, env...))` where `sndr_copy` is a
>    hypothetical non-`const` object equal to `sndr`. The above lambda returns an object `attrs` such
>    that, for a query object `q` and pack of subexpressions `as`, `attrs.query(q, as...)`
>    is ill-formed if `new_attrs.query(q, as...)` is ill-formed; otherwise, it has the
>    same type and value as `new_attrs.query(q, as...)`.
>
>    _Recommended practice_: Implementations should compute the results of queries
>    without actually calling `on_t::transform_sender`.
>
> :::


[Change [exec.then]{.sref} paragraphs 3 and 4 and add a new paragraph as
follows:]{.ednote}
...
>    [except that `sndr` is evaluated only once.]{.rm}
>
> 4. For `then`, `upon_error`, and `upon_stopped`, let _`set-cpo`_ be `set_value`, `set_error`, and
>    `set_stopped`, respectively. The exposition-only class template _`impls-for`_
>    ([exec.snd.expos]{.sref}) is specialized for _`then-cpo`_ as follows:
>
>    > ```cpp
>    > namespace std::execution {
>    >   template<>
...
>    >     @@[```static constexpr auto @_`get-attrs`_@ =```]{.add}@@
>    >       @@[```[](const auto& data, const auto& child) { return @_`see below`_@; };```]{.add}@@
>    >
>    >     static constexpr auto @_`complete`_@ = @[&hellip; as before &hellip;]{.blue}@;
>    >
>    >     template<class Sndr, class... Env>
>    >       static consteval void @_`check-types`_@();
>    >   };
...
> ::: add
>
> 5. Let `q` be a query object and let `as` be a pack of subexpressions. For subexpressions `child` and `fn`,
>    ```@_`impls-for`_@<@_`decayed-typeof`_@<@_`then-cpo`_@>>::@_`get-attrs`_@(fn, child)``` returns
>    an object `o` whose type satisfies _`queryable`_ such that `o.query(q, as...)` is
>    expression-equivalent to:
>
>    - [5.1]{.pnum} `D()` if `q` is `get_completion_domain<set_value_t>`, where `D` is the
>      type of the expression
>
>      > ```cpp
>      > @_`COMMON-DOMAIN`_@(@_`COMPL-DOMAIN`_@(@_`decayed-typeof`_@<@_`set-cpo`_@>, child, @_`FWD-ENV`_@(as)...),
>      >               @_`COMPL-DOMAIN`_@(set_value_t, child, @_`FWD-ENV`_@(as)...))
>      > ```
>
>    - [5.2]{.pnum} `D()` if `q` is `get_completion_domain<set_error_t>` and
>      ```@_`gather-signatures`_@<@_`decayed-typeof`_@<@_`set-cpo`_@>, CS, @_`is-nothrow`_@, conjunction>::value```
>      is `false`, where `CS` is
>      ```completion_signatures_of_t<decay_t<decltype((child))>, @_`FWD-ENV-T`_@(decltype((as)))...>```;
>      _`is-nothrow`_ is an alias template such that for a pack of types `Ts`, ```@_`is-nothrow`_@<Ts...>```
>      is `is_nothrow_invocable<decay_t<decltype((fn))>, Ts...>`; and `D` is the type of the
>      expression
>
>      > ```cpp
>      > @_`COMMON-DOMAIN`_@(@_`COMPL-DOMAIN`_@(@_`decayed-typeof`_@<@_`set-cpo`_@>, child, @_`FWD-ENV`_@(as)...),
>      >               @_`COMPL-DOMAIN`_@(set_error_t, child, @_`FWD-ENV`_@(as)...))
>      > ```
>
>    - [5.3]{.pnum} Otherwise, ```@_`FWD-ENV`_@(get_env(child)).query(q, as...)``` if
>      either of the following is `true`:
>
>      - [5.2.1]{.pnum} `q` is `get_completion_scheduler<set_value_t>` and _`set-cpo`_ is
>        `set_value`, or
>
>      - [5.2.2]{.pnum} `q` is not
>        ```get_completion_domain<@_`decayed-typeof`_@<@_`set-cpo`_@>>``` or
>        ```get_completion_scheduler<@_`decayed-typeof`_@<@_`set-cpo`_@>>```.
>
>    - [5.4]{.pnum} Otherwise, `o.query(q, as...)` is ill-formed.
>
> :::
...
>    > ```
>
>    Let `SetCPO` be the decayed type of _`set-cpo`_, let `Fn` be the decayed type of
>    `fn`, and let `Sndrs` be a pack of the parameters of the `tuple` specialization named
>    by ```@_`gather-signatures`_@<SetCPO, CS, @_`invoke-result`_@, tuple>```,
>    where `CS` is ```completion_signatures_of_t<decay_t<decltype((child))>, @_`FWD-ENV-T`_@(decltype((as)))...>```
>    and _`invoke-result`_ is an alias template such that ```@_`invoke-result`_@<Ts...>``` is
>    `invoke_result_t<Fn, Ts...>` for a pack `Ts`. The lambda above
>    returns an object `o` whose type satisfies _`queryable`_ such that, for a query
>    object `q` and pack of subexpressions `as`, let `let_env` be the pack
>    ```@_`JOIN-ENV`_@(@_`let-env`_@(child, as), @_`FWD-ENV`_@(as))```. Then
>     `o.query(q, as...)` is as follows:
>
>    - [8.1]{.pnum} If `q` is ```get_completion_domain<SetCPO>```,
>      `o.query(q, as...)` is expression-equivalent to `D()` where `D` is the type of the expression:
>
>      > ```cpp
>      > @_`COMMON-DOMAIN`_@(@_`COMPL-DOMAIN`_@(SetCPO, declval<Sndrs>(), let_env...)...)
>      > ```
>
>    - [8.2]{.pnum} Otherwise, if `q` is `get_completion_domain<set_error_t>` and
>      ```@_`gather-signatures`_@<SetCPO, CS, @_`is-nothrow`_@, conjunction>::value``` is
>      well-formed and `false`, `o.query(q, as...)` is ill-formed if `sizeof...(as) == 0`
>      is `true`; otherwise it is expression-equivalent to `D()`, where _`is-nothrow`_ is
>      the alias template
>
>      > ```cpp
>      > template<class... Ts>
>      >   using @_`is-nothrow`_@ = bool_constant<
>      >     (noexcept(auto(declval<Ts>())) &&...) &&
>      >     noexcept(connect(invoke(declval<Fn>(), declval<decay_t<Ts>&>()...),
>      >                      @_`receiver-archetype`_@()))>;
>      > ```
>
>      where ```@_`receiver-archetype`_@``` is an empty class type such that
>      ```receiver_of<@_`receiver-archetype`_@, U>``` is `true` for any specialization of
>      `completion_signatures` `U` and such that the type of
>      ```@_`receiver-archetype`_@().get_env()``` is `decltype((as...[0]))` [This assumes
>      the adoption of [@P3388R2], forwarded to LWG during the Kona 2025
>      meeting.]{.ednote}; and where `D` is the type of the following expression:
>
>      > ```cpp
>      > @_`COMMON-DOMAIN`_@(@_`COMPL-DOMAIN`_@(SetCPO, child, @_`FWD-ENV`_@(as)...),
>      >               @_`COMPL-DOMAIN`_@(set_error_t, child, @_`FWD-ENV`_@(as)...),
>      >               @_`COMPL-DOMAIN`_@(set_error_t, declval<Sndrs>(), let_env...)...)
>      > ```
>
>    - [8.3]{.pnum} Otherwise, if `q` is `get_completion_domain<set_error_t>` or
>      `get_completion_domain<set_stopped_t>`, let `Tag` be the template parameter of the
>      type of `q`. Then `o.query(q, as...)` is `D()` where `D()` is the type of the
>      following expression:
>
>      > ```cpp
>      > @_`COMMON-DOMAIN`_@(@_`COMPL-DOMAIN`_@(Tag, child, @_`FWD-ENV`_@(as)...),
>      >               @_`COMPL-DOMAIN`_@(Tag, declval<Sndrs>(), let_env...)...)
>      > ```
>
> :::
>
...
[Change [exec.when.all]{.sref} paragraphs 9 and 10 as follows:]{.ednote}
...
> ```
>
> 7. Let `Is` be the pack of integral template arguments of the `integer_sequence`
>    specialization denoted by ```@_`indices-for`_@<Sndr>```.
>
> 8. _Effects_: Equivalent to: [&hellip; as before &hellip;]{.blue}
>
> ::: rm
>
...
>     ::: add
>
>     Let ```@_`WHEN-ALL-DOMAIN`_@(Tag, env...)``` be the expression
>     ```@_`COMMON-DOMAIN`_@(@_`COMPL-DOMAIN`_@(Tag, child, env...)...)```.
>     The lambda expression above returns an object `attrs` such that
>
>     - [10.1]{.pnum} `attrs.query(get_completion_domain<set_value_t>, env...)` is
>       expression-equivalent to ```@_`WHEN-ALL-DOMAIN`_@(set_value_t, env...)```.
>
>     - [10.2]{.pnum} `attrs.query(get_completion_domain<set_error_t>, env...)` is
>       expression-equivalent to:
>
>       > ```cpp
>       > @_`COMMON-DOMAIN`_@(@_`WHEN-ALL-DOMAIN`_@(set_value_t, env...),
>       >               @_`WHEN-ALL-DOMAIN`_@(set_error_t, env...),
>       >               @_`WHEN-ALL-DOMAIN`_@(set_stopped_t, env...))
>       > ```
>
>     - [10.3]{.pnum} `attrs.query(get_completion_domain<set_stopped_t>, env...)` is
>       expression-equivalent to: [This assumes the adoption of [@P3887R0], which was
>       forwarded to LWG at the Kona 2025 meeting.]{.ednote}
>
>       > ```cpp
>       > @_`COMMON-DOMAIN`_@(@_`WHEN-ALL-DOMAIN`_@(set_value_t, env...),
>       >               @_`WHEN-ALL-DOMAIN`_@(set_stopped_t, env...))
...
>    >     @@[```static constexpr auto @_`get-attrs`_@ =```]{.add}@@
>    >       @@[```[](const auto&, const auto& child) noexcept -> decltype(auto) {```]{.add}@@
>    >         @@[```@_see below_@```]{.add}@@
>    >       @@[```};```]{.add}@@
>    >
>    >     template<class Sndr, class... Env>
...
> ::: add
>
> 4. For subexpressions `ign` and `child`, pack of subexpressions `as`, and query object
>    `q`, ```@_`impls-for`_@<stopped_as_optional_t>::@_`get-attrs`_@(ign, child)```
>    returns an object `o` such that `o.query(q, as...)` is expression-equivalent to:
>
>    - [4.1]{.pnum} `D()` if `q` is `get_completion_domain<set_value_t>`, where `D` is
>      the type of the expression
>
>      > ```cpp
>      > @_`COMMON-DOMAIN`_@(@_`COMPL-DOMAIN`_@(set_value_t, child, @_`FWD-ENV`_@(as)...),
>      >               @_`COMPL-DOMAIN`_@(set_stopped_t, child, @_`FWD-ENV`_@(as)...))
>      > ```
>
>    - [4.2]{.pnum} ```@_`FWD-ENV`_@(get_env(child)).query(q, as...)``` if `q` is not
>      `get_completion_domain<set_stopped_t>`, `get_completion_scheduler<set_value_t>`,
>      or `get_completion_scheduler<set_stopped_t>`.
>
>    - [4.3]{.pnum} Otherwise, `o.query(q, as...)` is ill-formed.
>
> :::
>
> 5. Let `sndr` and `env` be subexpressions such that `Sndr` is `decltype((sndr))` and `Env`
...
[Change [exec.stopped.err]{.sref} paragraphs 2-3 as follows:]{.ednote}
...
>
> 3. The exposition-only class template _`impls-for`_ ([exec.snd.expos]{.sref}) is
>    specialized for `stopped_as_error_t` as follows:
> 
>    > ```cpp 
>    > namespace std::execution {
>    >   template<>
>    >   struct @_`impls-for`_@<stopped_as_error_t> : @_`default-impls`_@ {
>    >     static constexpr auto @_`get-attrs`_@ =
...
>
> 4. For subexpressions `ign` and `child`, pack of subexpressions `as`, and query object
>    `q`, ```@_`impls-for`_@<stopped_as_error_t>::@_`get-attrs`_@(ign, child)```
>    returns an object `o` such that `o.query(q, as...)` is expression-equivalent to:
>
>    - [4.1]{.pnum} `D()` if `q` is `get_completion_domain<set_error_t>`, where `D` is
>      the type of the expression
>
>      > ```cpp
>      > @_`COMMON-DOMAIN`_@(@_`COMPL-DOMAIN`_@(set_error_t, child, @_`FWD-ENV`_@(as)...),
>      >               @_`COMPL-DOMAIN`_@(set_stopped_t, child, @_`FWD-ENV`_@(as)...))
>      > ```
>
>    - [4.2]{.pnum} ```@_`FWD-ENV`_@(get_env(child)).query(q, as...)``` if `q` is not
>      `get_completion_domain<set_stopped_t>`, `get_completion_scheduler<set_error_t>`,
>      or `get_completion_scheduler<set_stopped_t>`.
>
>    - [4.3]{.pnum} Otherwise, `o.query(q, as...)` is ill-formed.
>
...
> 10. The name `associate` denotes a pipeable sender adaptor object. For subexpressions
>     `sndr` and `token`:
>
>     - [10.1]{.pnum} If `decltype((sndr))` does not satisfy `sender`, or
...
[Change [exec.sync.wait]{.sref} paragraphs 4-9 as follows:]{.ednote}
...
>    `this_thread​::​sync_wait(sndr)` is [expression-]{.rm}equivalent to the following,
>    except that `sndr` is evaluated only once:
>
>    > ```cpp
>    > @@[```run_loop loop;```]{.add}@@
>    > @@[```@_`sync-wait-env`_@ env{&loop};```]{.add}@@
>    > @@[```auto domain = get_completion_domain<set_value_t>(get_env(sndr), env);```]{.add}@@
>    > @[`return`]{.add}@ apply_sender(@@[```@_`get-domain-early`_@(sndr)```]{.rm}[`domain`]{.add}@@, sync_wait, sndr@[, env]{.add}@)@[`;`]{.add}@
>    > ```
>
>    _Mandates_:
>
...
>
> 5. Let _`sync-wait-state`_ and _`sync-wait-receiver`_ be the following exposition-only
>    class templates: [Note the addition of `&` to the declaration of the
>    ```@_`sync-wait_state`_@::loop``` data member.]{.ednote}
>
>    > ```cpp
>    > namespace std::this_thread {
>    >   template<class Sndr>
>    >   struct @_`sync-wait-state`_@ {                                      // exposition only
>    >     execution::run_loop@[` & `]{.add}@ loop;                                  // exposition only
>    >     exception_ptr error;                                        // exposition only
>    >     @_`sync-wait-result-type`_@<Sndr> result;                         // exposition only
>    >   };
>    >
>    >   template<class Sndr>
>    >   struct @_`sync-wait-receiver`_@ {                                   // exposition only
>    >     using receiver_concept = execution::receiver_t;
>    >     @_`sync-wait-state`_@<Sndr>* state;                               // exposition only
>    >
>    >     template<class... Args>
...
>    >
>    >     @_`sync-wait-env`_@ get_env() const noexcept { return {&state->loop}; }
>    >   };
>    > }
>    > ```
>
> [Leave paragraphs 6-8 as-is]{.ednote}
>
> 9. For [a]{.rm} subexpression[s]{.add} `sndr` [and `env`]{.add}, let `Sndr` be
>    `decltype((sndr))`. If ```sender_to<Sndr, @_`sync-wait-receiver`_@<Sndr>>``` is
>    `false` [or if the type of `env` is not _`sync-wait-env`_]{.add}, the expression
>    ```sync_wait.apply_sender(sndr@[`, env`]{.add}@)``` is ill-formed; otherwise, it is
>    equivalent to:
>
>    > ```cpp
>    > @_`sync-wait-state`_@<Sndr> state@@[```{*env.@_`loop`_@}```]{.add}@@;
...
[Change [exec.sync.wait.var]{.sref} as follows:]{.ednote}
...
>    > @@[```@_`sync-wait-env`_@ env{&loop};```]{.add}@@
>    > @@[```auto domain = get_completion_domain<set_value_t>(get_env(sndr), env);```]{.add}@@
>    > @[`return`]{.add}@ apply_sender(@@[```@_`get-domain-early`_@(sndr)```]{.rm}[`domain`]{.add}@@, sync_wait_with_variant, sndr@[, env]{.add}@)@[`;`]{.add}@
>    > ```
>
>    _Mandates_:
>
...
>
> 2. [For subexpressions `sndr` and `env`, let `Sndr` be `decltype(into_variant(sndr))`.
>    If ```sender_to<Sndr, @_`sync-wait-receiver`_@<Sndr>>``` is `false` or if the type of
>    `env` is not _`sync-wait-env`_,]{.add} [T]{.rm}[t]{.add}the expression
>    ```sync_wait_with_variant.apply_sender(sndr@[`, env`]{.add}@)``` [is ill-formed;
>    otherwise, it]{.add} is equivalent to:
>
>    > ```cpp
>    > using result_type = @_`sync-wait-with-variant-result-type`_@<Sndr>;
>    > if (auto opt_value = sync_wait@[`.apply_sender`]{.add}@(into_variant(sndr)@[`, env`]{.add}@)) {
...
# Removing algorithm customization

This section discusses the implications of removing algorithm customizability.

Removing algorithm customization is fairly straightforward in most regards. The
sender algorithm customization point objects would directly return the result of
calling _`make-sender`_ instead of passing it to `transform_sender`.

However, there are a few parts of `std::execution` that need special care when
making the algorithms non-customizable.
...
when the user requests it and when the underlying execution context supports it.
...
This option is essentially the _status quo_, except that as discussed in
[](#the-parallel-scheduler), this aspect of the `parallel_scheduler` is currently
...
more general algorithm customization mechanism is available.
...
is _implementing_ the standard algorithms, they can make the implementations do whatever
they want.
...
5. Change the functions `get_completion_signatures`{.cpp} ([exec.getcomplsigs]{.sref}) and
   ...
   * If the sender has a tag with an exposition-only _`transform-sender`_ member function,
     pass the sender to this function with the receiver's environment and then call
     `connect` on the resulting sender. This preserves the behavior of calling
...
13. From the `scheduler` concept, remove the required expression:
    ...
    scheduler, but not `inline_scheduler`. See [](#inline_scheduler-improvements) for the
    motivation behind these changes, but in short: the `inline_scheduler` will not know
    where it completes in C++26, but it will in C++29 when we add back algorithm
    customization.
...
A paper targetting C++29 would propose that we extend the `get_completion_scheduler` query
...
customization. Prior to C++29, there would be no `get_*domain` queries, hence the addition
of those queries in C++29 would not affect any existing schedulers. And the domain queries
would be so-called "forwarding" queries, meaning they will automatically be passed through
...
Since C++29 would support only late (`connect`-time) customization, customizing
...
As described in [](#algorithm-dispatching-in-connect), `connect` will compute two domains,
the "starting" domain and the (value) "completion" domain:
...
The `connect` CPO would use these two domains to optionally apply "starting" and
"completing" transformations to the sender prior to calling `.connect(rcvr)` on it. This
will be a conforming extension since no schedulers in C++26 will have a custom domain tag,
and hence no sender transformations.
...
of `bulk`, `just()`, would not know where it will complete in C++26. There is no plumbing
...
All of the above is true also for `task_scheduler`, which merely adds an indirection to
the call to `connect`. After the changes suggested by the "Remove" option, the
`task_scheduler` accelerates `bulk` in the same way as `parallel_scheduler`.
