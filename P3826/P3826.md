...
* Integrate feedback from LWG wording review, 2026-03-25.
* Rebase the wording of [exec.let]{.sref} on @P3373R3.
* For consistency with @P3143R3, in [exec.on]{.sref}, remove the two uses of the
  `write_env` algorithm in the definition of the `on(sndr, sch, closure)` algorithm.
* **BUG FIX:** In [exec.on]{.sref}, fix how the `on(sndr, sch, closure)` algorithm
  computes the "current" scheduler.

## R4

* Fix typo in [exec.queries.expos]/p3: `o.query(tag, args...)` should be
  `q.query(tag, args...)`.
* Fix the return type of the exposition-only _`call-with-default`_ helper.
* Add back the definition of _`RECURSE-QUERY`_ that was inadvertantly dropped in R3.
* Clarify specifications of _`TRY-QUERY`_ and `get_domain` to ensure that extra arguments
  are always evaluated.
* Fix formatting of numbered paragraphs.

## R3

* Addresses feedback from the LEWG review of R2 (see [](#design-review-feedback)).
   - Adds a `get_completion_domain<>(attrs, envs...)` query that defaults to
     `get_completion_domain<set_value_t>(attrs, envs...)`.
...
> [1]{.pnum} [...] For the default implementation of the algorithm that produced `sndr`,
...
>     constexpr @[`sender`]{.rm}@ decltype(auto) transform_sender(
...
> ```@_`AS-CONST`_@(q).query(tag)```{.cpp} otherwise except that `args...` is evaluated.
>
...
> type of `tag` is `get_scheduler_t` or `get_domain_t`, and `q.query(tag, args...)`
> otherwise.
...
>
> - [2.3]{.pnum} `default_domain()`, except that `env` is evaluated.
>
> :::
>
> [3]{.pnum} `forwarding_query(execution​::​get_domain)` is a core constant expression and has
> value `true`.
...
>
> [3]{.pnum} `forwarding_query(execution​::​get_scheduler)` is a core constant expression and has
> value `true`.
...
>
> [1]{.pnum} ```get_completion_scheduler<@_`completion-tag`_@>```{.cpp} obtains the
> completion scheduler associated with a completion tag from a sender's attributes.
>
> ::: add
>
> [2]{.pnum} For subexpression `sch1` and pack `envs`, let `sch2` be
> ```@_`TRY-QUERY`_@(sch1, get_completion_scheduler<set_value_t>, envs...)``` and let
> ```@_`RECURSE-QUERY`_@(sch1, envs...)``` be expression-equivalent to `sch1` if `sch2` is
> ill-formed or if `sch1` and `sch2` have the same type and compare equal; otherwise,
> ```@_`RECURSE-QUERY`_@(sch2, envs...)```.
>
> :::
>
...
> ```get_completion_scheduler<@_`completion-tag`_@>(q@[`, envs...`]{.add}@)```{.cpp} is
> ill-formed if _`completion-tag`_ is not one of `set_value_t`, `set_error_t`, or
> `set_stopped_t`. Otherwise, ```get_completion_scheduler<@_`completion-tag`_@>(q@[`,
> envs...`]{.add}@)```{.cpp} is expression-equivalent to
>
> ::: rm
>   
> > ```cpp
...
>   if that expression is well-formed, except that `envs...` is evaluated only once.
>   
> - [3.2]{.pnum} Otherwise, `auto(q)` if the type of `q` satisfies `scheduler` and
>   `envs` is not an empty pack, except that `envs...` is evaluated.
>   
...
> ::: add
>
> [4]{.pnum} For a type `Tag`, subexpression `sndr`, and pack `envs`, let `CS` be
> `completion_signatures_of_t<decay_t<decltype((sndr))>, decltype((envs))...>`. If both
> `get_completion_scheduler<Tag>(get_env(sndr), envs...)` and `CS` are well-formed
> and ```CS().@_`count-of`_@(Tag()) == 0``` is `true`, the program is ill-formed.
>
> :::
>
> [5]{.pnum} Let _`completion-fn`_ be a completion function ([exec.async.ops]{.sref}); let
> _`completion-tag`_ be the associated completion tag of _`completion-fn`_; let `args`
> [and `envs`]{.add} be [a pack]{.rm} [packs]{.add} of subexpressions; and let `sndr` be a
...
> ```get_completion_domain<@_`completion-tag`_@>(attrs, envs...)```{.cpp} is ill-formed if
> _`completion-tag`_ is not one of `void`, `set_value_t`, `set_error_t`, or `set_stopped_t`.
> Otherwise, ```get_completion_domain<@_`completion-tag`_@>(attrs, envs...)```{.cpp} is
> expression-equivalent to ```@_`MANDATE-NOTHROW`_@(D())```, where `D` is:
>
> - [2.1]{.pnum} The type of
>   ```@_`TRY-QUERY`_@(attrs, get_completion_domain<@_`completion-tag`_@>, envs...)``` if
>   that expression is well-formed.
>
> - [2.2]{.pnum} Otherwise, the type of ```get_completion_domain<set_value_t>(attrs, envs...)```
>   if _`completion-tag`_ is `void`.
>
> - [2.3]{.pnum} Otherwise, the type of
>   ```@_`TRY-QUERY`_@(get_completion_scheduler<@_`completion-tag`_@>(attrs, envs...), get_completion_domain<set_value_t>, envs...)```
>   if that expression is well-formed.
>
> - [2.4]{.pnum} Otherwise, `default_domain` if `scheduler<decltype((attrs))>` is `true` and
>   `envs` is not an empty pack.
>
> - [2.5]{.pnum} Otherwise, ```get_completion_domain<@_`completion-tag`_@>(attrs, envs...)```
>   is ill-formed.
>
> [3]{.pnum} For a type `Tag`, subexpression `sndr`, and pack `envs`, let `CS` be
> `completion_signatures_of_t<decay_t<decltype((sndr))>, decltype((envs))...>`. If both
> `get_completion_domain<Tag>(get_env(sndr), envs...)` and `CS` are well-formed
> and ```CS().@_`count-of`_@(Tag()) == 0``` is `true`, the program is ill-formed.
>
> [4]{.pnum} Let _`completion-fn`_ be a completion function ([exec.async.ops]); let
> _`completion-tag`_ be the associated completion tag of _`completion-fn`_; let `args`
...
> well-formed and denotes a domain tag `D`. If an asynchronous operation created by
...
> [1]{.pnum} The `scheduler` concept defines the requirements of a scheduler type
...
> > [&hellip; as before &hellip;]{.blue}
>
> [5]{.pnum} For a given scheduler expression `sch`, [if]{.add} the expression
> ```get_completion_scheduler<set_value_t>(get_env(schedule(sch)))```{.cpp} [is
> well-formed, it]{.add} shall compare equal to `sch`.
>
> [6]{.pnum} For a given scheduler expression `sch` [, type `T`, and pack of
> subexpressions `envs`]{.add}, [if the expression ```get_domain(sch)```{.cpp} is
> well-formed, then the expression ```get_domain(get_env(schedule(sch)))```{.cpp} is
> also well-formed and has the same type.]{.rm} [the following expressions are
> either both ill-formed, or both well-formed with the same type:]{.add}
>
> ::: add
>
> - [6.1]{.pnum} ```get_completion_domain<T>(sch, envs...)```
>
> - [6.2]{.pnum} ```get_completion_domain<T>(get_env(schedule(sch)), envs...)```
>
> Likewise, the following expressions are either both ill-formed, or both
> well-formed with the same type and value:
>
> - [6.3]{.pnum} ```get_completion_scheduler<T>(sch, envs...)```
>
> - [6.4]{.pnum} ```get_completion_scheduler<T>(get_env(schedule(sch)), envs...)```
>
> :::
>
> [7]{.pnum} [&hellip; as before &hellip;]{.blue}
>
...
3. Each completion operation can potentially be evaluated on one of several different
   ...
   ```Ds@~_T_~@``` be a pack comprised of the set of domain tags associated with the execution agents that
   could potentially evaluate any of the operation's completions with tag `T`. If there are no potentially
   ...
   [Let `sndr2` be the sender `then(sndr, fn)`. `sndr2` has the same `set_value` completion domain tag
   as `sndr`, but if `fn`'s evaluation is potentially throwing, `sndr2`'s `set_error` completion
   domain tag would be the _`COMMON-DOMAIN`_ of `sndr`'s value and error completion domain tags,
...
4. If `sndr` can determine that all of its completion operations with tag `T` happen on
   execution agents associated with a particular scheduler `sch` (as determined by the
   ...
   is well-formed and has the type and value of `sch`; otherwise, it is ill-formed.

   [Let `sndr2` be the sender from the example above. The `set_value` completion scheduler
   of `sndr2` is the `set_value` completion scheduler of `sndr`, if any. But `sndr2` can only report
   a `set_error` completion scheduler when invocations of `fn` are not potentially
   throwing or when `sndr` has no `set_error` completions. When `fn` can throw, `sndr2` could
...
> [3]{.pnum} For a query object `q` [and]{.rm}[,]{.add} a subexpression `v`[, and a pack
> of subexpressions `args`]{.add}, ```@_`MAKE-ENV`_@(q, v)``` is an expression `env` whose type
> satisfies _`queryable`_ such that the result of ```env.query(q@[`, args...`]{.add}@)```
> has a value equal to `v` ([concepts.equality]{.sref}). Unless otherwise stated, the object to
> which ```env.query(q@[`, args...`]{.add}@)``` refers remains valid while `env` remains
> valid.
...
> ::: add
>
> [6]{.pnum} For a pack of subexpressions `domains`, ```@_`COMMON-DOMAIN`_@(domains...)``` is
...
> types removed.
> 
> [7]{.pnum} For a type `Tag`, subexpression `sndr`, and pack `envs`, ```@_`COMPL-DOMAIN`_@(Tag, sndr, envs)```
> is expression-equivalent to `D()` where `D` is the type of
> `get_completion_domain<Tag>(get_env(sndr), envs...)` if that expression is well-formed
> or `envs` is an empty pack, and `indeterminate_domain()` otherwise.
>
> :::
...
> [8]{.pnum} For a scheduler `sch`, [```@_`SCHED-ATTRS`_@(sch)``` is an expression o1 whose type
...
> `sch.query(get_domain)`.]{.rm} ```@_`SCHED-ENV`_@(sch)``` is an expression [`o2`]{.rm} [`o`]{.add} whose type
> satisfies _`queryable`_ such that ```@[`o2`]{.rm}[`o`]{.add}@.query(get_scheduler)``` is a prvalue with the
> same type and value as `sch`, and such that ```@[`o2`]{.rm}[`o`]{.add}@.query(get_domain)``` is
> expression-equivalent to `sch.query(get_domain)`.

<div style="color: blue">
\[ Editor's note: In [exec.snd.expos]{.sref}, make the following changes:<ul>
<li>Remove the prototype of the exposition-only _`completion-domain`_ function just before paragraph 8.</li>
<li>Remove paragraphs 8 and 9.</li>
<li>Remove the prototypes of the _`get-domain-early`_ and _`get-domain-late`_ functions.</li>
<li>Remove paragraphs 13 and 14.</li>
<li>After paragraph 26, change the declaration of the exposition-only class _`default-impls`_ as follows: \]</li></ul></div>
...
> ::: rm
>
> [35]{.pnum} The member ```@_`default-impls​`_@::@_`​get-attrs`_@``` is initialized with a
> callable object equivalent to the following lambda:
> 
> > ```cpp
> > [](const auto&, const auto&... child) noexcept -> decltype(auto) {
> >   if constexpr (sizeof...(child) == 1)
> >     return (@_`FWD-ENV`_@(get_env(child)), ...);
> >   else
> >     return env<>();
> > }
> > ```
...
>   constexpr decltype(auto) @_`call-with-default`_@(
>     Fn&& fn, Default&& value, Args&&... args) noexcept(@_`see below`_@);
> ```
>
> [52]{.pnum} Let `expr` be the expression
> `std::forward<Fn>(fn)(std::forward<Args>(args)...)` if that expression is well-formed;
> otherwise, it is `static_cast<Default>(std​::​forward<Default>(value))`.
>
> [53]{.pnum} _Returns_: `expr`.
>
> [54]{.pnum} _Remarks_: The expression in the `noexcept` clause is `noexcept(expr)`.
>
> [55]{.pnum}
> ```cpp
> template<class Tag>
...
> ```
>
> [56]{.pnum} For a subexpression `env`, ```@_`inline-attrs`_@<Tag>{}.query(get_completion_scheduler<Tag>, env)```
> is expression-equivalent to `get_scheduler(env)`.
>
> [57]{.pnum} For a subexpression `env`, ```@_`inline-attrs`_@<Tag>{}.query(get_completion_domain<Tag>, env)```
...
> ```
> 
> [2]{.pnum} _Mandates_: For each type `D` in `Domains...`, the expression
...
> 
> [3]{.pnum} _Returns_: `default_domain().transform_sender(Tag(), std::forward<Sndr>(sndr), env)`
> 
> [4]{.pnum} _Remarks_: For a pack of types `Ds`,
> `common_type_t<indeterminate_domain<Domains...>, indeterminate_domain<Ds...>>` is
> `indeterminate_domain<Us...>` where `Us` is a pack that contains each type in `Domains..., Ds...`
...
> `D` if `Domains` is an empty pack, and
...
>         @[`requires (sizeof...(Env) <= 1)`]{.rm}@
>       static constexpr @[`sender`]{.rm}@ decltype(auto) transform_sender(@[`Tag, `]{.add}@ Sndr&& sndr, const Env&@[`...`]{.rm}@ env)
>         noexcept(@_`see below`_@);
>
...
>   template<@[`class Domain,`]{.rm}@ sender Sndr, @_`queryable`_[`...`]{.rm}@ Env>
>       @[`requires (sizeof...(Env) <= 1)`]{.rm}@
>     constexpr @[`sender`]{.rm}@ decltype(auto) transform_sender(@[`Domain dom,`]{.rm}@ Sndr&& sndr, const Env&@[`...`]{.rm}@ env)
>       noexcept(@_`see below`_@);
> }
> ```
>
> ::: add
>
> [1]{.pnum} For a subexpression `s`, let ```@_`start-domain`_@``` be `D()` where
> `D` is the decayed type of `get_domain(env)` if that expression that is well-formed,
> and `default_domain` otherwise.
>
> [2]{.pnum} Let ```@_`completion-domain`_@(s)``` be `D()` where `D` is the decayed
> type of  `get_completion_domain<>(get_env(s), env)` if that
> is well-formed; otherwise, `D` is `default_domain`.
>
> :::
>
> [3]{.pnum} Let ```@_`transformed-sndr`_[`(dom, tag, s)`]{.add}@``` be the expression
>
> > ```cpp
...
> _`start-domain`_ if `tag` is `start`, and ```@_`completion-domain`_@(s2)```
> otherwise.]{.add}.
>
> ::: add
>
...
>
> > ```cpp
> > @_`transform-recurse`_@(@_`completion-domain`_@(sndr), set_value, sndr)
> > ```
>
> and let _`final-sndr`_ be the expression
>
> > ```cpp
> > @_`transform-recurse`_@(@_`start-domain`_@, start, @_`tmp-sndr`_@)
> > ```
>
> :::
>
...
> ```
>
> [1]{.pnum} Let _`except`_ be an rvalue subexpression of an unspecified class type
...
> > ```
>
> [2]{.pnum} [&hellip; as before &hellip;]{.blue}
>
> [3]{.pnum} [&hellip; as before &hellip;]{.blue}
>
> [4]{.pnum} Given a type `Env`, if `completion_signatures_of_t<Sndr, Env>` and
> `completion_signatures_of_t<Sndr>` are both well-formed, [they shall denote the
> same type]{.rm} [the former shall be a superset of the latter, with the difference
> corresponding to error or stopped completion
> operations]{.add}.


[Change [exec.connect]{.sref} paragraph 2 as follows:]{.ednote}

> [2]{.pnum} The name `connect` denotes a customization point object. For subexpressions
...
>   ([exec.fwd.env]{.sref}) [except that the
>   ```get_completion_scheduler<@_`completion-tag`_@>``` and
>   ```get_completion_domain<@_`completion-tag`_@>``` queries are handled as described in
>   [exec.snd.general]{.sref}]{.add}.
> 
> - [3.3]{.pnum} A parent sender with more than one child sender has an associated
>   attributes object equal to `env<>{}` [except that the
>   ```get_completion_scheduler<@_`completion-tag`_@>``` and
>   ```get_completion_domain<@_`completion-tag`_@>``` queries are handled as described in
>   [exec.snd.general]{.sref}]{.add}.
...
> [3]{.pnum} Otherwise, the expression `starts_on(sch, sndr)` is expression-equivalent to:
>
> > ```cpp
> > @@[`transform_sender(`]{.rm}@@
...
>
> [4]{.pnum} Let `out_sndr` and env be subexpressions such that `OutSndr` is
...
> :::
>
> ```starts_on.transform_sender(@[`set_value,`]{.add}@ out_sndr, env)``` is equivalent to:
>
> > ```cpp
...
>
>
> [1]{.pnum} [`schedule_from`]{.rm}[`continues_on`]{.add} schedules work dependent on the completion
...
>
> [2]{.pnum} The name [`schedule_from`]{.rm}[`continues_on`]{.add} denotes a customization point
...
> ill-formed.
>
> [3]{.pnum} Otherwise, the expression
> [`schedule_from(sch, sndr)`{.cpp}]{.rm}[`continues_on(sndr, sch)`{.cpp}]{.add} is
...
> ::: rm
>
> [5]{.pnum} The member ```@_`impls-for`_@<schedule_from_t>​::@_`​get-attrs`_@```{.cpp}
...
> :::
>
> [6]{.pnum} The member ```@_`impls-for`_@<@[`schedule_from_t`]{.rm}[`continues_on_t`]{.add}@>​::@_`​get-state`_@```{.cpp}
> is initialized with a callable object equivalent to the following lambda:
>
> [&hellip; as before &hellip;]{.blue}
>
> [&hellip; as before &hellip;]{.blue}
>
> [12]{.pnum} The member ```@_`impls-for`_@<@[`schedule_from_t`]{.rm}[`continues_on_t`]{.add}@>​::@_`complete`_@```{.cpp}
> is initialized with a callable object equivalent to the following lambda:
>
> [&hellip; as before &hellip;]{.blue}
>
> [13]{.pnum} Let `out_sndr` be a subexpression denoting a sender returned from
...
>
> [1]{.pnum} The name `schedule_from` denotes a customization point object. For some subexpression
> `sndr`, if `decltype((sndr))` does not satisfy `sender`, `schedule_from(sndr)` is
> ill-formed. Otherwise, the expression `schedule_from(sndr)` is expression-equivalent to
> ```@_`make-sender`_@(schedule_from, {}, sndr)```.
>
> [`schedule_from` is used by schedulers to control how to transition off of
> their schedulers' associated execution contexts.]{.note}
>
> :::
...
>
> [1]{.pnum} The `on` sender adaptor has two forms [&hellip; as before &hellip;]{.blue}
>
> [2]{.pnum} The name `on` denotes a [&hellip; as before &hellip;]{.blue}
>
> [3]{.pnum} Otherwise, if `decltype((sndr))` satisfies `sender`, the expression `on(sch, sndr)` is expression-equivalent to:
>
>
> ```cpp
> @[`transform_sender(`]{.rm}@
...
> [except that `sch` is evaluated only once.]{.rm}
>
> [4]{.pnum} For subexpressions `sndr`, `sch`, and `closure`, if
>
...
>
> [5]{.pnum} Let `out_sndr` and `env` be subexpressions, let `OutSndr` be `decltype((out_sndr))`,
...
> ::: rm
>
> [6]{.pnum} Otherwise: Let _`not-a-scheduler`_ be an unspecified empty class type.
>
...
> :::
>
> [8]{.pnum} [The]{.rm} [Otherwise, the]{.add} expression
...
> > ```
>
> [9]{.pnum} [ &hellip; as before &hellip; ]{.blue}
>
> [10]{.pnum} Let `out_sndr` be a subexpression denoting a sender returned from
> `on(sndr, sch, closure)` [ &hellip; as before &hellip; ]{.blue} `Calling start(op)`
> shall
>
> - [10.1]{.pnum} remember the current scheduler, which is [the first of the following
>   expressions that is well-formed:]{.rm}
>   [`get_completion_scheduler<set_value_t>(get_env(sndr), get_env(out_rcvr))`]{.add};
>
> ::: rm
>
>   - [10.1.1]{.pnum} `get_completion_scheduler<set_value_t>(get_env(sndr))`
>   - [10.1.2]{.pnum} `get_scheduler(get_env(rcvr))`;
>
> :::
>
> - [10.2]{.pnum} [ &hellip; as before &hellip; ]{.blue}

[Change [exec.then]{.sref} paragraphs 3 as follows:]{.ednote}

> [3]{.pnum} Otherwise, the expression ```@_`then-cpo`_@(sndr, f)``` is expression-equivalent to:
>
> > ```cpp
...
> [1]{.pnum} `let_value`, `let_error`, and `let_stopped` [&hellip; as before &hellip;]{.blue}
>
> [2]{.pnum} For `let_value`, `let_error`, and `let_stopped`, let _`set-cpo`_ be `set_value`,
> `set_error`, and `set_stopped`, respectively. Let the expression _`let-cpo`_ be one
> of `let_value`, `let_error`, or `let_stopped`.
> [The following sentence is added by @P3373R3:]{.ednote} Let _`let-tag`_ denote a unique,
> empty class type for each of `let_value`, `let_error`, and `let_stopped`.
> For [a subexpression]{.rm} [subexpressions]{.add}
...
>
> [3]{.pnum} The names `let_value`, `let_error`, and `let_stopped` denote [&hellip; as before &hellip;]{.blue}
>
...
> [except that `sndr` is evaluated only once.]{.rm}
>
> [This paragraph is added by @P3373R3:]{.ednote}
>
> [?]{.pnum} Let _`let-data`_ denote the following exposition-only class template:
>
> > ```c++
...
> > ```
>
> Then let the expression ```@_`let-cpo`_@.transform_sender(s, es...)``` be
> expression-equivalent to:
>
> > ```c++
> > @_`make-sender`_@(@_`let-tag`_@{}, @_`let-data`_@{s.template get<2>(), s.template get<1>()})
> > ```
>
> except that `s` is evaluated only once.
>
> [5]{.pnum} The exposition-only class [&hellip; as before &hellip;]{.blue}
>
...
>
> - [6.2]{.pnum} given a query object `q` [and pack of subexpressions `args`]{.add}, the
>   expression ```e.query(q@[`, args...`]{.add}@)``` is expression-equivalent to
>   ```env.query(q@[`, args...`]{.add}@)``` if that expression is valid; otherwise, if
>   the type of `q` satisfies _`forwarding-query`_, ```e.query(q@[`, args...`]{.add}@)```
>   is expression-equivalent to ```get_env(rcvr).query(q@[`, args...`]{.add}@)```;
>   otherwise, ```e.query(q@[`, args...`]{.add}@)``` is ill-formed.
>
...
>
> [The following paragraph is from @P3373R3:]{.ednote}
>
> [?]{.pnum} Let _`let-state`_ denote the following exposition-only class template:
>
> > ```c++
> > template<class Cpo, class Sndr, class Fn, class Rcvr, class ArgsVariant,
> >          class OpsVariant>
> > struct @_`let-state`_@ {
> >   using @_`env_t`_@ = decltype(@_`let-env`_@(declval<Sndr>()@[`, get_env(declval<Rcvr&>())`]{.add}@)); // exposition only
> >   Fn @_`fn`_@;                                                                       // exposition only
> >   @_`env_t`_@ @_`env`_@;                                                                   // exposition only
> >   ArgsVariant @_`args`_@;                                                            // exposition only
> >   OpsVariant @_`ops`_@;                                                              // exposition only
> >
> >   template<class Tag, class... Ts>
> >   constexpr void @_`impl`_@(Rcvr& rcvr, Tag tag, Ts&&... ts) noexcept
> >   {                                                  // exposition only
> >     @@[```@ &hellip; as in P3373 &hellip; @```]{.blue}@@
> >   }
> >
> >   struct @_`receiver`_@ { // exposition only
> >     @@[```@ &hellip; as in P3373 &hellip; @```]{.blue}@@
> >   };
> >
> >   using @_`op_t`_@ = connect_result_t<Sndr, receiver>; // exposition only
> >
> >   constexpr @_`let-state`_@(Sndr&& sndr, Fn fn, Rcvr& rcvr) // exposition only
> >     : @_`fn`_@(std::move(fn)), @_`env`_@(@_`let-env`_@(sndr@[`, get_env(rcvr)`]{.add}@)),
> >       @_`ops`_@(in_place_type<@_`op_t`_@>, std::forward<Sndr>(sndr),
> >           @_`receiver`_@{*this, rcvr})
> >   {}
> > };
> > ```
> 
> [leave paragraphs 8-13 unchanged]{.ednote}
>
> ::: rm
>
> [14]{.pnum} Let `sndr` and `env` be subexpressions, and let `Sndr` be `decltype((sndr))`. If
...
> :::
>
> [15]{.pnum} Let the subexpression `out_sndr` denote [&hellip; as before &hellip;]{.blue}

[Change [exec.bulk]{.sref} paragraph 3 and 4 as follows:]{.ednote}

> [3]{.pnum} Otherwise, the expression ```@_`bulk-algo`_@(sndr, policy, shape, f)``` is
> expression-equivalent to:
>
> > ```c++
...
>
> [4]{.pnum} Let `sndr` and `env` be subexpressions such that `Sndr` is `decltype((sndr))`. If
...
> [2]{.pnum} The names `when_all` and `when_all_with_variant` denote customization point objects.
...
> > ```
>
> [4]{.pnum} The exposition-only class template impls-for ([exec.snd.expos]) is specialized for `when_all_t` as follows:
> 
> > ```cpp
> > namespace std::execution {
> >   template<>
...
> ```
>
> [8]{.pnum} Let `Is` be the pack of integral template arguments of the `integer_sequence`
> specialization denoted by ```@_`indices-for`_@<Sndr>```.
>
> [9]{.pnum} _Effects_: Equivalent to: [&hellip; as before &hellip;]{.blue}
>
> ::: rm
>
> [10]{.pnum} _Throws_: Any exception thrown as a result of evaluating the _Effects_, or an exception
> of an unspecified type derived from `exception` when `CD` is ill-formed.
>
> [11]{.pnum} The member ```@_`impls-for`_@<when_all_t>​::@_`​get-attrs`_@``` is initialized with a
...
> [20]{.pnum} The expression `when_all_with_variant(sndrs...)` is expression-equivalent to:
>
> > ```cpp
> > @[`transform_sender(CD2(),`]{.rm}@ @_`make-sender`_@(when_all_with_variant, {}, sndrs...)@[`);`]{.rm}@
> > ```
>
> [21]{.pnum} Given subexpressions `sndr` and `env`, if
...
> [3]{.pnum} Otherwise, the expression `into_variant(sndr)` is expression-equivalent to:
>
> > ```cpp
...
[Change [exec.stopped.opt]{.sref} paragraph 2-4 as follows:]{.ednote}

> [2]{.pnum} The name `stopped_as_optional` denotes a pipeable sender adaptor object.
...
>
> [3]{.pnum} The exposition-only class template [&hellip; as before &hellip;]{.blue}
>
> [4]{.pnum} Let `sndr` and `env` be subexpressions such that `Sndr` is `decltype((sndr))` and `Env`
...
[Change [exec.stopped.err]{.sref} paragraph 1-3 as follows:]{.ednote}

> [1]{.pnum} `stopped_as_error` maps an input sender's stopped completion operation into an error
...
>
> [2]{.pnum} The name `stopped_as_error` denotes a pipeable sender adaptor object. For some
...
>
> [3]{.pnum} Let `sndr` and `env` be subexpressions such that `Sndr` is `decltype((sndr))` and
...
> [9]{.pnum} The name `associate` denotes a pipeable sender adaptor object. For subexpressions
> `sndr` and `token`:
>
...
> > ```
>
> [1]{.pnum} The class template `env` is used to construct a queryable object from several
...
> > ```
>
> [5]{.pnum} Let _`has-query`_ be the following exposition-only concept:
>
> > ```cpp
...
> > ```
>
> [6]{.pnum} Let _`fe`_ be the first element of ```@_envs_~0~@, @_envs_~1~@, @&hellip;@ @_envs_~_n_-1~@```{.cpp}
> such that the expression ```@_`fe`_@.query(q@[`, std::forward<Args>(args)...`]{.add}@)```{.cpp}
> is well-formed.
>
> [7]{.pnum} _Constraints_: ```(@_`has-query`_@<Envs, QueryTag@[`, Args...`]{.add}@> || ...)```{.cpp} is `true`{.cpp}.
>
> [8]{.pnum} _Effects_: Equivalent to: ```return @_`fe`_@.query(q@[`, std::forward<Args>(args)...`]{.add}@);```{.cpp}
>
> [9]{.pnum} _Remarks_: The expression in the `noexcept`{.cpp} clause is equivalent to
...
> [4]{.pnum} Let _`sch`_ be an expression of type _`run-loop-scheduler`_. The expression
> `schedule(sch)`{.cpp} has type _`run-loop-sender`_ and is not potentially-throwing if
> _`sch`_ is not potentially-throwing.
>
> ::: add
>
> [5]{.pnum} For type _`set-tag`_ other than `set_error_t`, the expression
...
> [3]{.pnum} Otherwise, the expression `affine_on(sndr, sch)`{.cpp} is expression-equivalent
...
> ::: rm
>
> [4]{.pnum} The exposition-only class template _`impls-for`_ ([exec.snd.expos]{.sref}) is
...
> [1]{.pnum} `inline_scheduler` is a class that models `scheduler` ([exec.sched]{.sref}). All
> objects of type `inline_scheduler` are equal. [For a subexpression `sch` of type
> `inline_scheduler`, a query object `q`, and a pack of subexpressions `args`, the
> expression `sch.query(q, args...)` is expression-equivalent to
> ```@_`inline-attrs`_@<set_value_t>().query(q, args...)```.]{.add}
>
> [2]{.pnum} _`inline-sender`_ is an exposition-only type that satisfies `sender`. The type
> ```completion_signatures_of_t<@_`inline-sender`_@>``` is
> `completion_signatures<set_value_t()>`.
>
> [3]{.pnum} Let `sndr` be an expression of type _`inline-sender`_, let `rcvr` be an expression such that
...
no longer accepts a domain tag. It computes the two domain tags, starting and completing, and
...
references:
  - id: P3143R3
    citation-label: P3143R3
    title: "Scheduler Affinity"
    author:
      - family: Kühl
        given: Dietmar
    email: dkuhl@bloomberg.net
    URL: https://isocpp.org/files/papers/P3941R3.html
  - id: P3373R3
    citation-label: P3373R3
    title: "Of Operation States and Their Lifetimes"
    author:
      - family: Leahy
        given: Robert
    email: rleahy@rleahy.ca
    URL: https://isocpp.org/files/papers/P3373R3.pdf
---
