...
[After subsection [exec.get.domain]{.sref}, add a new subsection with stable name
...
>
> 1. `get_domain_override` asks a queryable object for its associated execution
>   domain override tag.
>
> 2. The name `get_domain_override` denotes a query object. For subexpressions `attrs` and
>   `tag`, where `tag` is one of `set_value` or `start`, and pack of subexpressions `env`
>   `get_domain_override(attrs, tag, env...)` is expression-equivalent to
>   ```@_`MANDATE-NOTHROW`_@(@_`AS-CONST`_@(attrs).query(get_domain_override, tag, env...))```.
>
> 3. `forwarding_query(execution​::​get_domain_override)` is a core constant
...
>   >         @@[```return @_`MAKE-ENV`_@(get_completion_behavior, completion_behavior::inline_completion);```]{.add}@@
>   >       @@[```};```]{.add}@@
>   >
...
[Change [exec.read.env]{.srev} paragraph 3 as follows:]{.ednote}
...
>
>    - [5.4]{.pnum} ```attrs.query(get_domain_override, set_value, env...)``` is expression-equivalent
>      to `D()` where `D` is the type of ```get_completion_domain<set_value_t>(child, env...)```.
>
>    - [5.5]{.pnum} ```attrs.query(get_domain_override, start, env...)``` is expression-equivalent
>      to `D()` where `D` is the type of ```get_completion_domain<set_value_t>(data, env...)```.
>
>    - [5.6]{.pnum} For a pack of subexpressions `as` and a query object `q` whose type
>      satisfies _`forwarding-query`_ and is not `get_domain_override_t` or a
>      specialization of `get_completion_scheduler_t`, `get_completion_domain_t`, or
...
>    > ```
>
>    Given a pack of subexpressions `e`, let `E` be the pack ```decltype(e)``` and
...
>
>        > ```cpp
>        > get_completion_behavior<@_`completion-tag`_@>(get_env(schedule(data)), @_`FWD-ENV`_@(env)...)
>        > ```
>
>        `B` is the type of
>
>        > ```cpp
>        > get_completion_behavior<@_`completion-tag`_@>(get_env(child), @_`JOIN-ENV`_@(@_`SCHED-ENV`_@(data, env...), env)...)
>        > ```
>
>        and `C` is the type of
>
>        > ```cpp
>        > get_completion_behavior<@_`completion-tag`_@>(get_env(schedule(old_sch)), @_`FWD-ENV`_@(env)...)
>        > ```
>
>        where `old_sch` is ```@_`call-with-default`_@(get_scheduler, @_`not-a-scheduler`_@(), env)```.
>
