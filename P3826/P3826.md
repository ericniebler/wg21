...
- The async APIs on `bounded_queue` (`async_push`, `async_emplace`, `async_pop`) and the
  Asynchronous Concurrent Queue concept ([@P0260R19] proposal that targets for C++29).
...
design (described in [](#the-planned-fix)) as a polestar, this paper proposes wording
...
[](#the-planned-fix). It would not be the first time the Committee
...
This is the approach taken by the [](#proposed-resolution) below.
