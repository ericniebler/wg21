...
>        is [TODO (LucTeo: I'm not sure if we can easily compute the completion scheduler in case of an error)]{.blue}
>
>      - [5.1.3]{.pnum} ```attrs.query(get_completion_domain<set_value_t>, env...)```
>        is expression-equivalent to `get_domain(env...[0])`.
>
>      - [5.1.4]{.pnum} ```attrs.query(get_completion_domain<@_`completion-tag`_@>, env...)```
>        is expression-equivalent to
>        ```@_`COMMON-DOMAIN`_@(get_domain(env...[0]), get_completion_domain<decay_t<@_`completion-tag`_@>>(get_env(sndr), env...[0]))```,
>        when _`completion-tag`_ is one of `set_error_t` or `set_stopped_t`.
>
>      - [5.1.5]{.pnum} For a pack of subexpressions `as` and a query object `q`,
>        ```attrs.query(q, as...)``` is expression-equivalent to
>        ```@_`FWD-ENV`_@(get_env(child)).query(q, as...)```.
>
>    - [5.2]{.pnum} Otherwise, for a pack of subexpressions `as` and a query object `q`,
>      `attrs.query(q, as...)` is expression-equivalent to
>      ```@_`FWD-ENV`_@(get_env(child)).query(q, as...)```. [TODO: is there more to say
>      here?]{.blue}
>
> :::
...
>    > ```
>
>    Let `Sndrs` be a pack of the parameters of the `tuple` specialization named by
>    ```@_`gather-signatures`_@<@_`decayed-typeof`_@<@_`set-cpo`_@>, CS, @_`invoke-result`_@, tuple>```,
...
>    `invoke_result_t<decay_t<decltype((fn))>, Ts...>` for a pack `Ts`. The lambda above
>    returns an object `o` whose type satisfies _`queryable`_ such that, for a query
>    object `q` and pack of subexpressions `as`, `o.query(q, as...)` is expression
>    equivalent to:
>
>    - [8.1]{.pnum} `D()` if `q` is ```get_completion_domain<@_`decayed-typeof`_@<@_`set-cpo`_@>>```
>      where `D` is the type of the expression
>
>      > ```cpp
>      > @_`COMMON-DOMAIN`_@(@_`COMPL-DOMAIN`_@(@_`decayed-typeof`_@<@_`set-cpo`_@>,
>      >                            declval<Sndrs>(),
>      >                            @_`JOIN-ENV`_@(@_`let-env`_@(child, as), @_`FWD-ENV`_@(as))...)...)
>      > ```
>
>    - [8.2]{.pnum} `D()` if `q` is `get_completion_domain<set_error_t>` and
>      ```@_`gather-signatures`_@<@_`decayed-typeof`_@<@_`set-cpo`_@>, CS, @_`is-nothrow`_@, conjunction>::value```
>      is well-formed and `false`, where `CS` is
>      ```completion_signatures_of_t<decay_t<decltype((child))>, @_`FWD-ENV-T`_@(decltype((as...[0])))>```;
>      _`is-nothrow`_ is the alias template
>
>      > ```cpp
>      > template<class... Ts>
>      > using @_`is-nothrow`_@ = bool_constant<
>      >   (noexcept(auto(declval<Ts>())) &&...) &&
>      >   noexcept(connect(invoke(declval<decay_t<decltype((fn))>>(), declval<decay_t<Ts>&>()...),
>      >                    @_`receiver-archetype`_@<decltype((as...[0]))>()))>;
>      > ```
>
>      where ```@_`receiver-archetype`_@<T>``` is an empty class type such that
>      ```receiver_of<@_`receiver-archetype`_@<T>, completion_signatures<CS...>>``` is
>      `true` for any pack of completion signatures `CS` and such that the type of
>      ```@_`receiver-archetype`_@<T>().get_env()``` is `T` [This assumes the adoption of
>      [@P3388R2], forwarded to LWG during the Kona 2025 meeting.]{.ednote}; and where `D`
