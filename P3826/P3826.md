...
>
>    - [6.1]{.pnum} `Tag()(sch, as...)` if `Tag` is a specialization of
>      `get_completion_scheduler_t` or `get_completion_domain_t` and either of the
>      following is `true`:
>
>      - [6.1.1]{.pnum} The template parameter of `Tag` is `set_value_t`, or
>
>      - [6.1.2]{.pnum}
...
>
>    - [6.2]{.pnum} `D()` if `Tag` is ```get_completion_domain_t<@_`completion-tag`_@>```,
...
>      > ```
>
>    - [6.3]{.pnum} Otherwise, ```@_`FWD-ENV`_@(get_env(sndr)).query(Tag(), as...)``` if
>      either of the following is `true`:
>
>      - [6.3.1]{.pnum} `Tag` is not a specialization of `get_completion_scheduler_t` or
...
>        is `true`, where _`completion-tag`_ is the template parameter of `Tag`.
>
>    - [6.4]{.pnum} Otherwise, the expression `o.query(Tag(), as...)` is ill-formed.
>
> 9. For a scheduler `sch`, ```@_`SCHED-ENV`_@(sch)``` is an expression `o` whose type
>    satisfies _`queryable`_ such that
>
>    - [7.1]{.pnum} `o.query(get_scheduler)` is a prvalue with the same type and value as
>      `sch`,
>
>    - [7.2]{.pnum} `o.query(get_domain)` is expression-equivalent to
...
>      > ```
>
>    - [5.2]{.pnum} Otherwise, ```@_`FWD-ENV`_@(get_env(child)).query(q, as...)``` if
>      either of the following is `true`:
>
...
>
>    - [5.3]{.pnum} Otherwise, `o.query(q, as...)` is ill-formed.
>
> :::
