...
> >
> > - [3.2]{.pnum} Otherwise, `auto(q)` if the type of `q` satisfies `scheduler` and `sizeof...(envs) == 0`
> >   is `false`.
> >
...
[TODO: check that `prop` permits extra arguments]{.ednote}
...
> 6. For unqualified type `Tag` and subexpressions `sndr1`, `sndr2`, and pack `as`,
>    ```@_`SEQUENCE-ATTRS`_@(sndr1, sndr2)``` is an expression `o` whose type satisfies _`queryable`_
>    such that `o.query(Tag(), as...)` is expression-equivalent to:
>
>    - [6.1]{.pnum} `Tag()(sch, as...)` if `Tag` is a specialization of
>      `get_completion_scheduler` or `get_completion_domain`,
>
>    - [6.2]{.pnum} Otherwise, `attrs.query(Tag(), as...)` if `Tag` satisfies
>      _`forwarding-query`_,
>
>    - [6.3]{.pnum} Otherwise, `o.query(Tag(), as...)` is ill-formed.
>
...
> ::: add
>
> 6. For unqualified type `Tag` and subexpressions `sch`, `sndr`, and pack `as`,
...
>
>      - [6.1.2]{.pnum}
>        ```@_`never-completes-with`_@<decltype((sndr)), @_`completion-tag`_@, decltype(as)...>```
>        is `true`, where _`completion-tag`_ is the template parameter of `Tag`.
>
>    - [6.2]{.pnum} Otherwise, ```@_`FWD-ENV`_@(get_env(sndr)).query(Tag(), as...)``` if
>      `Tag` is not a specialization of `get_completion_scheduler_t` or
>      `get_completion_domain_t`.
>
>    - [6.3]{.pnum} Otherwise, the expression `o.query(Tag(), as...)` is ill-formed.
>
> 7. For a scheduler `sch`, ```@_`SCHED-ENV`_@(sch)``` is an expression `o` whose type
...
>    [See https://cplusplus.github.io/LWG/issue4368]{.ednote}
>
> 3. _Returns_: `e`.
>
...
>
>      - [5.1.5]{.pnum} For a pack of subexpressions `as` and a query object `q` whose
>        type satisfies _`forwarding-query`_, ```attrs.query(q, as...)``` is
>        expression-equivalent to ```get_env(child).query(q, as...)```.
>
...
> ::: add
>
> 5. Let `q` be a query object and let `as` be a pack of subexpressions. The lambda used
>    to initialize
>    ```@_`impls-for`_@<@_`decayed-typeof`_@<@_`then-cpo`_@>>::@_`get-attrs`_@``` returns
>    an object `o` whose type satisfies _`queryable`_ such that `o.query(q, as...)` is
>    expression-equivalent to:
>
>    - [5.1]{.pnum} `D()` if `q` is `get_completion_domain<set_value_t>`, where `D` is the type
>      ```common_type_t<@_`COMPL-DOMAIN`_@(@_`set-cpo`_@), @_`COMPL-DOMAIN`_@(set_value)>```
>      and where ```@_`COMPL-DOMAIN`_@(tag)``` is the type of the expression
>      ```get_completion_domain<decltype(auto(tag))>(get_env(child), as...)```.
>
>    - [5.2]{.pnum} Otherwise, ```@_`FWD-ENV`_@(get_env(child)).query(q, as...)``` if either of the following is `true`:
>
...
>    > auto fn = []<class... Ts>(@@[_`set_value_t`_]{.rm}[```@_`decayed-typeof`_@<@_`set-cpo`_@>```]{.add}@@(*)(Ts...)) {
>    >   if constexpr (!invocable<remove_cvref_t<@_`data-type`_@<Sndr>>, Ts...>)
>    >     throw @_`unspecified-exception`_@();
>    >   @[`if (sizeof...(Env) != 0 && !requires (Sndr& sndr, Env&... env) {`]{.add}@
>    >         @[`get_env(sndr).query(get_completion_domain<set_value_t>, env...); })`]{.add}@
...
>
>    - [2.1]{.pnum} ```@_`SCHED-ENV`_@(get_completion_scheduler<@_`decayed-typeof`_@<@_`set-cpo`_@>>(get_env(sndr)@[`, env`]{.add}@))```
>
>    ::: add
>
>    - [2.2]{.pnum} ```@_`MAKE-ENV`_@(get_domain, get_completion_domain<@_`decayed-typeof`_@<@_`set-cpo`_@>>(get_env(sndr), env))```
>
>    :::
>
>    ::: rm
...
>    `decltype((sndrs))...`, [let `tag` and `env` be subexpressions,]{.add} and let
>    [`CD`]{.rm}[```@_`when-all-domain`_@(tag, env)```]{.add} be the [type]{.rm}[expression]{.add}
>    [```common_type_t<decltype(@_`get-domain-early`_@(sndrs))...>```]{.rm}.
>    [```common_type_t<decltype(get_completion_domain<decay_t<decltype((tag))>>(sndrs, env))...>()```]{.add}.
...
>    ```decltype(@_`when-all-env`_@(declval<Env>()))```]{.add} is ill-formed.
>
...
>
>     ::: add
>     Let ```@_`common-domain`_@(domains...)``` be the expression `common_type_t<domains...>` if the
>        expression is well-formed, otherwise `default_domain()`.
>        [TODO: move this from here]{.blue}
>
>
>     For a type `tag` and a subexpression `env`, the lambda expression above returns an object `attrs` such
>     that
>
>     - [10.1]{.pnum} `get_completion_domain<set_value_t>(attrs, env)` is expression-equivalent
>       to ```@_`when-all-domain`_@(tag, env)```.
>
>     - [10.2]{.pnum} `get_completion_domain<set_error_t>(attrs, env)` is expression-equivalent
>       to ```@_`common-domain`_@(@_`when-all-domain`_@(set_value_t, env), @_`when-all-domain`_@(set_error_t, env))```.
>
>     :::
...
>    [except that `sndr` is only evaluated once.]{.rm}
>
> 3. Let `sndr` and `env` be subexpressions such that `Sndr` is `decltype((sndr))` and `Env`
