...
>    of an unspecified type derived from `exception` when [`CD`]{.rm} [the type
>    ```decltype(@_`when-all-env`_@(declval<Env>()))```]{.add} [TODO: this is wrong]{.blue}
>    is ill-formed.
>
...
[TODO: does `task` need sender attributes?]{.ednote}
