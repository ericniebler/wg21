...
>    `decltype((sndrs))...`, [let `env` be a subexpression,]{.add} and let
>    [`CD`]{.rm}[```@_`when-all-domain`_@(env)```]{.add} be the [type]{.rm}[expression]{.add}
>    [```common_type_t<decltype(@_`get-domain-early`_@(sndrs))...>```]{.rm}.
>    [```common_type_t<decltype(get_completion_domain<set_value_t>(sndrs, env))...>()```]{.add}.
...
>     ::: add
>
>     For a subexpression `env`, the lambda expression above returns an object `attrs` such
>     that
>
>     - [10.1]{.pnum} `get_completion_domain<set_value_t>(attrs, env)` is expression-equivalent
>       to ```@_`when-all-domain`_@(env)```.
>
