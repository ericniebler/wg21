...
> ::: add
>
> 6. For unqualified type `Tag` and subexpressions `sch`, `attrs`, and pack `as`,
>    ```@_`SCHED-ATTRS`_@(sch, attrs)``` is an expression `o` whose type satisfies _`queryable`_
>    such that `o.query(Tag(), as...)` is expression-equivalent to:
>
>    - [6.1]{.pnum} `Tag()(sch, as...)` if `Tag` is a specialization of
>      `get_completion_scheduler` or `get_completion_domain`,
...
>    ```@_`SCHED-ENV`_@(sch)``` is an expression `o` whose type satisfies _`queryable`_
>    such that
>
...
> ::: add
>
>    For a subexpression `env`, the lambda expression above returns an object `attrs` such that
>
>    - [2.4]{.pnum} ```attrs.query(get_completion_domain<@_`set-cpo`_@>, env)``` is
>      expression-equivalent to ```get_domain(env)```.
>
>    - [2.5]{.pnum} ```attrs.query(get_completion_scheduler<@_`set-cpo`_@>, env)``` is
>      expression-equivalent to ```get_scheduler(env)```.
>
> :::
...
>        is `true`.
>
>      - [5.1.3]{.pnum} Otherwise,
>        ```attrs.query(get_completion_scheduler<@_`completion-tag`_@>, env...)``` is ill-formed.
>
...
>        is `true`.
>
>      - [5.2.3]{.pnum} Otherwise,
>        ```attrs.query(get_completion_domain<@_`completion-tag`_@>, env...)``` is ill-formed.
>
...
>    >       [](const auto& data, const auto& child) noexcept -> decltype(auto) {
>    >         @@[```return @_`JOIN-ENV`_@(@_`SCHED-ATTRS`_@(data), @_`FWD-ENV`_@(get_env(child)));```]{.rm}@@
>    >         @@[```return @_`SCHED-ATTRS`_@(data, get_env(child));```]{.add}@@
>    >       };
>    >   };
>    > }
>    > ```
